...
@NamedQueries({
    ...
    @NamedQuery(
            name="getMyAllYear",
            query="SELECT m.year FROM Money AS m WHERE m.person = :person AND m.delete_flag = 0 GROUP BY m.year"
            ),
    @NamedQuery(
            name="getMyPersonId",
            query="SELECT COUNT(m) FROM Money AS m WHERE m.person = :person AND m.delete_flag = 0"
            ),
    @NamedQuery(
            name="getMyAllMoneys",
